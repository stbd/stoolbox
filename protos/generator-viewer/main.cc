...
typedef boost::chrono::nanoseconds TimeDurationNano;
...
bool parseParameters(int argc, char * argv[], Parameters & params)
{
    ...
    options.add_options()
        ...
        //        ("w", po::value<unsigned>(&params.w), "Window width")
        //        ("h", po::value<unsigned>(&params.h), "Window height")
        ;
...
bool initShader(stb::Shader & shader, ShaderVars & shaderVars, const Parameters & params, stb::Log & log)
{
    ...
    const glm::mat4 projectionMatrix = glm::perspective<float>(fovy, static_cast<float>(params.w) / static_cast<float>(params.h), 0.1f, 10.0f);
...
public:
    ...
    {
        stb::initFileConsoleLogger("generator.log", 0);
        ...
        if ((m_window = SDL_CreateWindow("Generator viewer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, params.w, params.h,
    ...
    bool generateModel()
    {
        ...
        switch (m_generatedEntity) {
        case 0:
        {
            stb::ModelData model = stb::generateSphere(m_subdivides);
        ...
        case 1:
        {
            stb::ModelData model = stb::generateCube(m_subdivides);
            logModelData(model, m_log);
            stb::initVao(m_vao, model, layoutInfo);
            break;
        }
        default:
            assert(!"Generated model invalid");
        }
        return true;
    }
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (event.key.keysym.scancode == SDL_SCANCODE_ESCAPE) {
                    ...
                    m_generatedEntity = (m_generatedEntity + 1) % MAX_NUMBER_OF_ENTIES;
                    generateModel();
