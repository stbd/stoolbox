...
struct Attribute
{
    ...
    {
        //This is only safe for objects in scale of unit cube
        //so do not copy this comperator
...
stb::ModelData stb::generateSphere(const U subdivides, const float radius)
{
    ...
    const U numberOfVertice = 8 * (std::pow(4, subdivides) * 3); //Guess, a poor one
    ...
    return stb::ModelData({ stb::ModelData::AttributeElement(element) },
                          (const char *)&indiceContainer[0],
                          sizeof(IndiceContainter::value_type) * indiceContainer.size(),
                          sizeof(indiceContainer[0]),
                          stb::ModelData::TRIANGLE
